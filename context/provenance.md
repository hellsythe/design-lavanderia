...
> **Accessibility adjustment:** `--accent` was darkened from `#0D9488` to `#0F766E` and `--warning` from `#D97706` to `#B45309` so white text on accent and warning text on `--warning-soft` meet WCAG AA contrast (≥ 4.5:1). Source artifacts preserve the original hex values.
